    ...
    protected static function boot()
    {
        ...
        static::creating(function ($user) {
            if (empty($user->email)) {
                ...
                $emailDomain = Str::slug(($user->login_type ?? 'social'), '_') . '.local';
                $email = $emailLocal . '@' . $emailDomain;
                $counter = 1;

                while (self::withTrashed()->where('email', $email)->exists()) {
            ...
            if (empty($user->username)) {
                $baseUsername = Str::slug(Str::before(($user->email ?? ($user->first_name ?? 'user')), '@'), '_');
