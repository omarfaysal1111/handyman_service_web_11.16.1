    ...
    public function socialLogin(Request $request)
    {
        ...
        if ($user_data != null) {
            ...
            if (empty($input['username'])) {
                $baseUsername = Str::slug(Str::before(($input['email'] ?? ($input['first_name'] ?? 'user')), '@'), '_');
            ...
            if (empty($input['email'])) {
                $emailLocal = Str::slug(($input['uid'] ?? $input['username'] ?? Str::random(8)), '_');
                $emailLocal = $emailLocal !== '' ? $emailLocal : 'user';
                $emailDomain = Str::slug(($input['login_type'] ?? 'social'), '_') . '.local';
                $email = $emailLocal . '@' . $emailDomain;
                $counter = 1;

                while (User::withTrashed()->where('email', $email)->exists()) {
                    $email = $emailLocal . '_' . $counter . '@' . $emailDomain;
                    $counter++;
                }
            ...
            $input['last_name'] = !empty($input['last_name']) ? $input['last_name'] : 'User';
